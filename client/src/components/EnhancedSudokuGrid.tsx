...
export default function EnhancedSudokuGrid({
  ...
  const getCellClasses = (row: number, col: number) => {
    ...
    return cn(
      `${cellSize} border border-gray-400/30 flex items-center justify-center font-bold cursor-pointer transition-colors`,
  ...
  return (
    <div className="flex justify-center w-full h-full">
      <div className="relative w-full h-full max-w-full max-h-full flex items-center justify-center">
        <div 
          className={cn(
            "grid gap-0 border-2 border-black rounded-md sm:rounded-lg overflow-hidden bg-white",
...
function getGameSpecificBorders(
  ...
  if (size === 16) {
    if (col === 3 || col === 7 || col === 11) classes.push("border-r-2 border-black dark:border-gray-500");
    if (row === 3 || row === 7 || row === 11) classes.push("border-b-2 border-black dark:border-gray-500");
  } else if (size === 9) {
    if (col === 2 || col === 5) classes.push("border-r-2 border-black dark:border-gray-500");
    if (row === 2 || row === 5) classes.push("border-b-2 border-black dark:border-gray-500");
  } else if (size === 6) {
